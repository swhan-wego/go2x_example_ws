...
- `src/llm_robot_agent` - LLM 기반 로봇 에이전트 노드
...
```bash
# 워크스페이스 루트에서
uv pip install -e src/llm_robot_agent
...
```bash
ros2 topic pub --once /build_cmd std_msgs/msg/String "{data: '{\"type\": \"sit\"}'}"
...
```bash
ros2 topic pub --once /build_cmd std_msgs/msg/String "{data: '{\"type\": \"stand_up\"}'}"
...
```bash
ros2 topic pub --once /build_cmd std_msgs/msg/String "{data: '{\"type\": \"hello\"}'}"
...
```bash
ros2 topic pub --once /build_cmd std_msgs/msg/String "{data: '{\"type\": \"damp\"}'}"
