...
def generate_launch_description():
    ...
    node_terminal_keyboard = Node(
        package="webrtc_common",
        executable="webrtc_terminal_keyboard",
        name="terminal_keyboard_node",
        output="screen",
        arguments=["--ros-args", "-p", f"keymap:={keymap_candidate}"],
        prefix=[venv_python],
        # prefix=f"gnome-terminal -- {venv_python}",
    )

    return LaunchDescription([node_terminal_keyboard])
