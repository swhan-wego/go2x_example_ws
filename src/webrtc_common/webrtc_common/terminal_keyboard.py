...
class KeyboardHandler(Node):
    def __init__(self):
        ...
        self.default_cooldown = float(
            self.get_parameter("default_cooldown").get_parameter_value().double_value
        )
    ...
    def start(self):
        ...
        def stdin_thread():
            self.get_logger().info(
                "[!] Terminal input listener started (using /dev/tty)"
            )

            try:
                tty_file = open("/dev/tty", "rb", buffering=0)
            except OSError as e:
                self.get_logger().error(f"Cannot open /dev/tty: {e}")
                return
            ...
            try:
                ...
                self.get_logger().error(f"tty.setraw error: {e}")
                tty_file.close()
                return

            def read_key():
                """Read one keypress (including escape sequences) from /dev/tty."""
            ...
            try:
                while not self._stop_event.is_set():
                    ...
                    if name is not None:
                        ...
                        if name == "esc":
                            self.get_logger().info(
                                "ESC pressed, stopping listener and sending SIGINT."
                            )
                        ...
                        try:
                            ...
                            if current_time - last_time >= self.default_cooldown:
                                ...
                                remaining = self.default_cooldown - (
                                    current_time - last_time
                                )
                                self.get_logger().info(
                                    f"[!] '{name}' 키 쿨타임 중... ({remaining:.2f}초 남음)"
                                )
                        except Exception as e:
...
def main(args=None):
    ...
    try:
        ...
        try:
            ...
            if (
                hasattr(keyboard_handler_node, "_stdin_thread")
                and keyboard_handler_node._stdin_thread.is_alive()
            ):
