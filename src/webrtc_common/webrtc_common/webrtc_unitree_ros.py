...
COMMAND_MAP = {
    "move":          lambda cmd: {"api_id": SPORT_CMD["Move"],
                      "parameter": {"x": cmd["x"], "y": cmd["y"], "z": cmd["z"]}},
    "euler":         lambda cmd: {"api_id": SPORT_CMD["Euler"],
                      "parameter": {"x": cmd["roll"], "y": cmd["pitch"], "z": cmd["yaw"]}},
    "bodyheight":    lambda cmd: {"api_id": SPORT_CMD["BodyHeight"],
                      "parameter": {"z": cmd["height"]}},
    "balance_stand": lambda cmd: {"api_id": SPORT_CMD["BalanceStand"]},
    "stand_down":    lambda cmd: {"api_id": SPORT_CMD["StandDown"]},
    "stand_up":      lambda cmd: {"api_id": SPORT_CMD["StandUp"]},
    "damp":          lambda cmd: {"api_id": SPORT_CMD["Damp"]},
    "hello":         lambda cmd: {"api_id": SPORT_CMD["Hello"]},
    "sit":           lambda cmd: {"api_id": SPORT_CMD["Sit"]},
    "pose":          lambda cmd: {"api_id": SPORT_CMD["Pose"],
                      "parameter": {"flag": True}},
}
...
class UnitreeRobotNode(Node):
    def __init__(self):
        ...
        self.is_hotspot = self.get_parameter('is_hotspot').get_parameter_value().bool_value
        # 파라미터에서 속도값 읽기
        self.move_speed = self.get_parameter('move_speed').get_parameter_value().double_value
        self.side_speed = self.get_parameter('side_speed').get_parameter_value().double_value
        ...
        # 토픽 구독 설정
        self.pressed_sub = self.create_subscription(Int32, 'keyboard_pressed', self.pressed_callback, 10)
        self.released_sub = self.create_subscription(Int32, 'keyboard_released', self.released_callback, 10)
    ...
    async def send_command(self, cmd):
        try:
            ...
            await self.conn.datachannel.pub_sub.publish_request_new(
                RTC_TOPIC["SPORT_MOD"], builder(cmd)
            )
    ...
    def pressed_callback(self, msg):
        ...
        try:
            ...
            char = None # 특수키인 경우 처리 필요
            
        self.vx, self.vy, self.vz = 0, 0, 0

        # 키 매핑 (ASCII 기준 및 이전 정의 기준)
        move_flag = False
        euler_flag = False
        height_flag = False
        ...
        if key_val == 32: # Space
            ...
            match char:
                case 'w': self.vx, move_flag = self.move_speed, True
                case 's': self.vx, move_flag = -self.move_speed, True
                case 'a': self.vy, move_flag = self.side_speed, True
                case 'd': self.vy, move_flag = -self.side_speed, True
                case 'q': self.vz, move_flag = self.turn_speed, True
                case 'e': self.vz, move_flag = -self.turn_speed, True
                case 'r': self.roll, euler_flag = self.roll + 0.1, True
                case 'f': self.roll, euler_flag = self.roll - 0.1, True
                case 't': self.pitch, euler_flag = self.pitch + 0.1, True
                case 'g': self.pitch, euler_flag = self.pitch - 0.1, True
                case 'y': self.yaw, euler_flag = self.yaw + 0.1, True
                case 'h': self.yaw, euler_flag = self.yaw - 0.1, True
                case 'u': self.body_height, height_flag = self.body_height + 0.03, True
                case 'j': self.body_height, height_flag = self.body_height - 0.03, True
                case '1': self.command_queue.sync_q.put({"type": "damp"})
                case '2': self.command_queue.sync_q.put({"type": "hello"})
                case '3': self.command_queue.sync_q.put({"type": "sit"})
                case 'v': self.command_queue.sync_q.put({"type": "pose"})
                case 'z': self.command_queue.sync_q.put({"type": "stand_down"})
                case 'x': self.command_queue.sync_q.put({"type": "stand_up"})
                case 'c': self.command_queue.sync_q.put({"type": "balance_stand"})

        # 이동 명령 큐에 추가
        if move_flag:
            self.command_queue.sync_q.put({"type": "move", "x": self.vx, "y": self.vy, "z": self.vz})
        if euler_flag:
            # Roll: Value range [-0.75~0.75 ] (rad) Pitch: Value range [-0.75~0.75 ] (rad) Yaw: Value range [-0.6~0.6 ] (rad)
            self.roll = max(min(self.roll, 0.75), -0.75)
            self.pitch = max(min(self.pitch, 0.75), -0.75)
            self.yaw = max(min(self.yaw, 0.6), -0.6)
            self.command_queue.sync_q.put({"type": "euler", "roll": self.roll, "pitch": self.pitch, "yaw": self.yaw})
        if height_flag:
            # Height: Relative value to the default fuselage height, value [-0.18~0.03 ] (m)
            self.body_height = max(min(self.body_height, 0.03), -0.18)
            self.command_queue.sync_q.put({"type": "bodyheight", "height": self.body_height})
